...
class brandItemWindow():
    ...
    def newBrandSubmit(self):
        if pStore.fileData.addNewBrand(self.brandEntry.getValues()[0]): #do following only when brand add is a success
            ...
            self.newBrand.childRoot.withdraw()
    ...
    def newItemSubmit(self):
        if pStore.fileData.addNewItem(self.itemEntry.getValues()): #do following only when item add is a success
            ...
            self.newItem.childRoot.withdraw()

    def changedItemSubmit(self):
        pStore.fileData.itemList[self.vKeyItem]=self.itemChange.getValues()
        pStore.fileData.writeItem()
        self.itemObject.brandItemListRefresh()
        self.oldItem.childRoot.withdraw()

    def addBrand(self):
        self.newBrand=pStore.childFrame(self.itemRoot,'ADD BRAND','300x200')
        pStore.appLabel(self.newBrand.childRoot,[('New Brand :',20,50)])
        self.brandEntry=pStore.appEntrybox(self.newBrand.childRoot,[(100,50)])
        oBrand=pStore.appButtons(self.newBrand.childRoot,[['SUBMIT',[50,100],[2,15]]])
    ...
    def addItem(self):
        if self.itemObject.brandVal == None:
            ...
            self.newItem=pStore.childFrame(self.itemRoot,'ADD ITEM','400x300')
            pStore.appLabel(self.newItem.childRoot,[('Brand :',50,50),('Item name :',50,100),('Model No :',50,150),('HSN Code :',50,200)])
            self.itemEntry=pStore.appEntrybox(self.newItem.childRoot,[(150,50,20,1),(150,100),(150,150),(150,200)])
            self.itemEntry.setValues([self.itemObject.brandVal,'','',''])
            obj=pStore.appButtons(self.newItem.childRoot,[['SUBMIT',[150,250],[2,15]]])
    ...
    def changeItem(self):
        if self.itemObject.itemVal == None:
            ...
            self.oldItem=pStore.childFrame(self.itemRoot,'CHANGE ITEM','400x300')
            pStore.appLabel(self.oldItem.childRoot,[('Brand :',50,50),('Item name :',50,100),('Model No :',50,150),('HSN Code :',50,200)])
            self.itemChange=pStore.appEntrybox(self.oldItem.childRoot,[(150,50,20,1),(150,100),(150,150),(150,200)])
            ...
            obj=pStore.appButtons(self.oldItem.childRoot,[['SUBMIT',[150,250],[2,15]]])
